...
def train_epoch(model, ds, esm_cache, masked_cache, crit, opt, dev, cfg):
    ...
    for idx in tqdm(np.random.permutation(len(ds)), desc="Train", leave=False):
        ...
        if p is None:
            continue
        try:
            gs = protein_to_graphs_v2_masked(
            ...
            if cfg.dual_dir_weight > 0:
                gr = protein_to_graphs_v2_masked(
                    make_reverse(p), esm_cache=esm_cache, masked_esm_cache=masked_cache,
                    env_radius=cfg.env_radius, edge_cutoff=cfg.edge_cutoff, max_neighbors=cfg.max_neighbors)
                if gr:
...
def main():
    ...
    pa.add_argument("--output_dir", default="runs/leen_v2_masked")
    ...
    pa.add_argument("--masked_esm_cache", required=True, help="site-masked ESM cache (exact anti-symmetry)")
    pa.add_argument("--dual_dir_weight", type=float, default=0.5, help="reverse-direction accuracy auxiliary weight")
    pa.add_argument("--output_dir_default", default="runs/leen_v2_masked")
    args = pa.parse_args()

    cfg = Cfg(
        data_root=args.data_root, esm_cache_path=args.esm_cache,
        masked_esm_cache_path=args.masked_esm_cache, dual_dir_weight=args.dual_dir_weight,
