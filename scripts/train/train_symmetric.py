...
def make_reverse(protein):
    """Mutant-scaffold reverse mutation: S becomes mutant, mutation swapped mut->wt.
    With site masking, this is an INDEPENDENT input that must give exactly -ΔΔĜ_fwd."""
    import copy
    p = copy.deepcopy(protein)
    S = np.asarray(p["S"]).reshape(-1).copy()
    at = np.asarray(p["append_tensors"]).copy()
    ...
    p["S"] = S.reshape(np.asarray(protein["S"]).shape)
    p["append_tensors"] = at
    return p


def train_epoch(model, ds, esm_cache, masked_cache, crit, opt, dev, cfg):
    model.train()
    tot, n = 0.0, 0
    ...
    for idx in tqdm(np.random.permutation(len(ds)), desc="Train", leave=False):
        ...
        if p is None:
            continue
        try:
            gs = protein_to_graphs_v2_symmetric(
                p, esm_cache=esm_cache, sym_esm_cache=masked_cache,
                env_radius=cfg.env_radius, edge_cutoff=cfg.edge_cutoff, max_neighbors=cfg.max_neighbors,
            )
            if not gs:
                continue
            ...
            if cfg.dual_dir_weight > 0:
                gr = protein_to_graphs_v2_symmetric(
                    make_reverse(p), esm_cache=esm_cache, sym_esm_cache=masked_cache,
                    env_radius=cfg.env_radius, edge_cutoff=cfg.edge_cutoff, max_neighbors=cfg.max_neighbors)
                if gr:
                    br = collate_v2(gr).to(dev)
                    if br.ddg is not None:
                        pred_rev = model(br)["ddg_pred"]
                        ld["total"] = ld["total"] + cfg.dual_dir_weight * torch.nn.functional.mse_loss(pred_rev, br.ddg)
...
@torch.no_grad()
def eval_ds(model, ds, esm_cache, masked_cache, dev, cfg, name=""):
    model.eval()
    preds, tgts = [], []
...
def main():
    ...
    pa.add_argument("--output_dir", default="runs/leen_v2_symmetric")
    ...
    pa.add_argument("--encoder", default="egnn", choices=["egnn", "gvp", "painn"])
    pa.add_argument("--max_epochs", type=int, default=200)
    pa.add_argument("--lr", type=float, default=1e-4)
    pa.add_argument("--warmup_epochs", type=int, default=5)
    pa.add_argument("--no_scheduler", action="store_true")
    pa.add_argument("--esm_dropout", type=float, default=0.25,
                    help="Fraction of training proteins with ESM dropped (default 0.25)")
    pa.add_argument("--sym_esm_cache", required=True, help="symmetrized ESM cache (exact anti-symmetry, full info)")
    pa.add_argument("--dual_dir_weight", type=float, default=0.5, help="reverse-direction accuracy auxiliary weight")
    pa.add_argument("--output_dir_default", default="runs/leen_v2_symmetric")
    args = pa.parse_args()

    cfg = Cfg(
        data_root=args.data_root, esm_cache_path=args.esm_cache,
        masked_esm_cache_path=args.sym_esm_cache, dual_dir_weight=args.dual_dir_weight,
        ...
        warmup_epochs=args.warmup_epochs, use_scheduler=not args.no_scheduler,
    ...
    masked_cache = torch.load(cfg.masked_esm_cache_path, map_location="cpu", weights_only=False)
    log.info(f"  {len(masked_cache)} masked-site embeddings cached")
    ...
    model = LEENv2(
        ...
        symmetrize_site=cfg.symmetrize_site,
    ).to(dev)
    ...
    if cfg.use_scheduler:
        warm = LinearLR(opt, start_factor=0.1, total_iters=cfg.warmup_epochs)
        cos = CosineAnnealingLR(opt, T_max=max(1, cfg.max_epochs - cfg.warmup_epochs), eta_min=cfg.lr * 0.05)
        sched = SequentialLR(opt, [warm, cos], milestones=[cfg.warmup_epochs])

    best, pat = -1.0, 0
    for ep in range(1, cfg.max_epochs + 1):
        t0 = time.time()
        loss, esm_pct = train_epoch(model, train_ds, esm_cache, masked_cache, crit, opt, dev, cfg)
        vm = eval_ds(model, val_ds, esm_cache, masked_cache, dev, cfg, "val")
        log.info(
            f"Epoch {ep:3d} | loss={loss:.4f} | val_sp={vm['spearman']:.4f} | "
            f"esm_used={esm_pct:.0f}% | lr={opt.param_groups[0]['lr']:.2e} | {time.time()-t0:.1f}s"
        )
