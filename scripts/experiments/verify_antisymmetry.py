...
Accuracy will be poor on an un-retrained model — that's fine; this checks the PROPERTY, not accuracy.
  ...
  PYTHONPATH=/backup/new_work python verify_antisymmetry.py \
    ...
    --masked_esm_cache masked_esm_cache_full.pt \
...
def score(model, protein, masked_cache, device):
    gs = protein_to_graphs_v2_masked(protein, esm_cache=None, masked_esm_cache=masked_cache)
    if not gs:
        return None
...
def main():
    ...
    pa.add_argument("--ckpt", required=True); pa.add_argument("--masked_esm_cache", required=True)
    ...
    for pidx in range(min(len(dataset), args.n)):
        ...
        for mi in range(num_muts):
            p_single = copy.deepcopy(protein)
            p_single["mut_ids"] = protein["mut_ids"][mi:mi+1]
            p_single["append_tensors"] = protein["append_tensors"][mi:mi+1]
            if torch.is_tensor(protein["ddG"]) and protein["ddG"].dim() > 0:
                p_single["ddG"] = protein["ddG"][mi:mi+1]
            elif not torch.is_tensor(protein["ddG"]) and len(protein["ddG"]) > 1:
                p_single["ddG"] = protein["ddG"][mi:mi+1]

            fwd = score(model, p_single, masked_cache, device)
            rev = score(model, apply_mutation_to_S(p_single), masked_cache, device)
    ...
    if not violations:
        print("=> No valid mutation evaluations found.")
        return
